<?php
// =====================================================
// edit.php — Halaman & Proses Edit Transaksi
// =====================================================
...
// Ambil data transaksi milik user (validasi ownership)
$stmtGet = mysqli_prepare($conn,
    "SELECT * FROM transaksi WHERE id = ? AND user_id = ? LIMIT 1");
mysqli_stmt_bind_param($stmtGet, 'ii', $id, $userId);
...
$errors = [];
$data   = $transaksi; // data yang akan ditampilkan di form (di-overwrite saat POST error)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update $data dengan input baru agar form bisa di-repopulate
    $data = array_merge($data, compact('jenis', 'jumlah', 'tanggal', 'keterangan'));
    ...
    $fotoNamaBaru = $transaksi['foto_nota']; // default: pertahankan foto lama
    if (!empty($_FILES['foto_nota']['name'])) {
        ...
        if (!in_array($ext, $allowedExt)) {
            $errors[] = 'Foto nota hanya boleh JPG, JPEG, atau PNG.';
        } elseif ($file['size'] > $maxSize) {
            $errors[] = 'Ukuran foto nota maksimal 2 MB.';
        } elseif ($file['error'] !== UPLOAD_ERR_OK) {
            $errors[] = 'Terjadi kesalahan saat mengunggah foto.';
        } else {
            $namaTemp = time() . '_' . preg_replace('/[^a-zA-Z0-9._-]/', '', basename($file['name']));
            if (move_uploaded_file($file['tmp_name'], __DIR__ . '/uploads/' . $namaTemp)) {
                // Hapus foto lama
                if (!empty($transaksi['foto_nota'])) {
                    $oldPath = __DIR__ . '/uploads/' . $transaksi['foto_nota'];
                    if (file_exists($oldPath)) unlink($oldPath);
                }
                $fotoNamaBaru = $namaTemp;
            } else {
    ...
    if (empty($errors)) {
        ...
        $stmt = mysqli_prepare($conn,
            "UPDATE transaksi SET jenis=?, jumlah=?, tanggal=?, keterangan=?, foto_nota=?
             WHERE id=? AND user_id=?");
        mysqli_stmt_bind_param($stmt, 'sdsssis',
            $jenis, $jumlahDecimal, $tanggal, $keterangan, $fotoNamaBaru, $id, $userId);
...
<body class="bg-slate-950 min-h-screen font-sans antialiased text-white">
    ...
    <nav class="sticky top-0 z-50 bg-slate-900/95 backdrop-blur-md border-b border-slate-800">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <span class="text-sm text-slate-400 hidden sm:block">👤 <?= htmlspecialchars($currentUser['nama']) ?></span>
                <a href="logout.php" id="btn-logout" class="text-sm text-slate-400 hover:text-red-400 transition-colors flex items-center gap-1.5">
    ...
    <main class="max-w-2xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-xl font-bold text-white">Edit Transaksi</h1>
                <p class="text-sm text-slate-400">Perbarui data transaksi #<?= $id ?></p>
            </div>
        </div>
                ...
                <div class="space-y-5">
                        ...
                        <div class="grid grid-cols-2 gap-3" id="jenis-selector">
                            ...
                            <label class="jenis-option cursor-pointer">
                                <input type="radio" name="jenis" value="pengeluaran" class="sr-only"
                                    <?= ($data['jenis'] === 'pengeluaran') ? 'checked' : '' ?>>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" id="jumlah" name="jumlah" min="1" step="1" required
                                ...
                                class="w-full pl-10 pr-4 py-2.5 bg-slate-800 border border-slate-600 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 transition-all">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="tanggal" name="tanggal" required readonly
                            ...
                            class="w-full px-3.5 py-2.5 bg-slate-800 border border-slate-600 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 transition-all cursor-pointer">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="keterangan" name="keterangan" required maxlength="255"
                            ...
                            class="w-full px-3.5 py-2.5 bg-slate-800 border border-slate-600 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 transition-all">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mb-3 p-3 bg-slate-800 border border-slate-700 rounded-xl">
                            <p class="text-xs text-slate-400 mb-2">Foto saat ini:</p>
                            <img src="uploads/<?= htmlspecialchars($data['foto_nota']) ?>"
                        ...
                        <div id="upload-area"
                            class="relative border-2 border-dashed border-slate-600 rounded-xl p-5 text-center hover:border-indigo-500/60 transition-colors cursor-pointer">
                            <input type="file" id="foto_nota" name="foto_nota" accept=".jpg,.jpeg,.png"
                                class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                            <div id="upload-placeholder">
                                <svg class="w-7 h-7 text-slate-500 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div id="preview-container" class="hidden">
                                <img id="preview-img" src="" alt="Preview foto baru" class="max-h-32 mx-auto rounded-lg object-contain">
